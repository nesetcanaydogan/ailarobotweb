# AILA - Roadmap

## Phase 1: Foundation & Environment Setup
**Goal:** A functional monorepo structure with shared design tokens and a basic layout shell.
**Plans:** 4 plans
- [ ] 01-01-PLAN.md — Initialize monorepo workspace and shared validation package.
- [ ] 01-02-PLAN.md — Setup backend foundation with Express and Prisma.
- [ ] 01-03-PLAN.md — Setup frontend foundation with Vite, React 19, and Tailwind v4.
- [ ] 01-04-PLAN.md — Implement UI shell (Navbar, Footer, Shell).

## Phase 2: Core UI Components (The 'Internal Dashboard')
- [ ] Implement Glassy Feature Cards.
- [ ] Implement Primary & Secondary Buttons.
- [ ] Create the Hero Section Layout.
- [ ] Implement the Contact/Lead Form.

## Phase 3: 3D Integration & Scroll Magic
- [ ] Setup Three.js Canvas with React Three Fiber.
- [ ] Integrate Placeholder AILA Model.
- [ ] Implement Scroll-Synced Animation Logic (Rotation, Positioning).
- [ ] Fine-tune Z-index interactions (AILA moving behind/over elements).

## Phase 4: Agentic Demo & Backend
- [ ] Setup GPT-4o Proxy Endpoint.
- [ ] Implement Demo Input & Processing Logic.
- [ ] Create Demo Result Visualization.
- [ ] Setup Database & Lead Capture persistence.

## Phase 5: Polish & Deployment
- [ ] Add atmospheric glows and subtle gradients.
- [ ] Performance optimization (3D textures, code splitting).
- [ ] Final UI/UX Audit.
- [ ] Deployment to Vercel/Railway.
